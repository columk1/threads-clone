...
export default function UserProfile({ initialUser, children }: { initialUser: PublicUser | PostUser, children: React.ReactNode }) {
      ...
      <div className="flex w-full flex-1 flex-col md:rounded-t-3xl md:border-[0.5px] md:border-gray-4 md:bg-active-bg">
        ...
        <div className="flex w-full flex-col items-center justify-center gap-4 px-6 py-5 text-[15px] font-normal">
          ...
          <div className="flex w-full gap-2">
            <FollowButton isAuthenticated={isAuthenticated} isFollowed={'isFollowed' in user ? user.isFollowed : false} onToggleFollow={handleToggleFollow} />
            <button type="button" className="h-9 w-full rounded-lg border border-gray-5 px-4 text-[15px] font-semibold transition active:scale-95 disabled:opacity-30">Mention</button>
          </div>
        </div>
        <ProfileNavigation />
        {children}
